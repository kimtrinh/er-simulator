...
function generateMarkdown(data: DebriefData): string {
  const lines: string[] = [
    `# MediSim ER — Case Debrief`,
    ``,
    ...
    `| Category | Score |`,
    `|----------|-------|`,
    `| History & Data Collection | ${data.performanceBreakdown.historyDataCollection}% |`,
    `| Differential Diagnosis | ${data.performanceBreakdown.differentialDiagnosis}% |`,
    `| Medical Management | ${data.performanceBreakdown.medicalManagement}% |`,
    `| Communication Efficiency | ${data.performanceBreakdown.communicationEfficiency}% |`,
    ``,
  ...
  for (const ev of data.criticalEvents) {
    const tag = ev.type === 'positive' ? 'Good Call' : ev.type === 'negative' ? 'Pitfall' : 'Note';
    lines.push(`### ${ev.event} [${tag}]`);
    lines.push(`- **Your action:** ${ev.userAction}`);
    lines.push(`- **Optimal action:** ${ev.optimalAction}`);
    lines.push(`- *${ev.feedback}*`);
    lines.push(``);
  }
  ...
  if (data.missedOpportunities.length > 0) {
    ...
    lines.push(``);
    for (const m of data.missedOpportunities) lines.push(`- ${m}`);
    lines.push(``);
  }

  if (data.cmeLearningPoints.length > 0) {
    lines.push(`## CME Learning Points`);
    lines.push(``);
    for (const p of data.cmeLearningPoints) lines.push(`- ${p}`);
    lines.push(``);
  }

  lines.push(`---`);
  lines.push(`*Generated by MediSim ER on ${new Date().toLocaleDateString()}*`);
  return lines.join('\n');
}

function downloadMarkdown(data: DebriefData) {
  const md = generateMarkdown(data);
  const blob = new Blob([md], { type: 'text/markdown;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `medisim-debrief-${new Date().toISOString().slice(0, 10)}.md`;
  a.click();
      ...
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex justify-center gap-4 pt-8">
            <button
                ...
                className="group relative px-8 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg font-bold shadow-lg transition-all overflow-hidden border border-slate-600"
            >
